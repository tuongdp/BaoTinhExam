...
export const QuestionRenderer = ({ question, value, onChange }: Props) => {
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-start justify-between gap-3 space-y-0 p-3 sm:p-4">
        <CardTitle className="text-base leading-6 sm:text-lg">{question.content}</CardTitle>
        <Badge variant="secondary" className="shrink-0">{question.points} điểm</Badge>
      </CardHeader>
      ...
      {["MULTIPLE_CHOICE", "TRUE_FALSE", "IMAGE_CHOICE", "VIDEO_CHOICE", "AUDIO_CHOICE"].includes(question.type) ? (
        <RadioGroup value={typeof value === "string" ? value : ""} onValueChange={onChange}>
          {(question.options ?? []).map((option) => (
            <Label key={option.id} className="flex min-h-12 cursor-pointer items-center gap-3 rounded-md border border-input p-3 hover:bg-accent hover:text-accent-foreground">
      ...
      {question.type === "ESSAY" ? (
        <Textarea className="min-h-40" value={String(value ?? "")} onChange={(event) => onChange(event.target.value)} />
      ) : null}
      </CardContent>
    </Card>
  );
